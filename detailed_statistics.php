...
?>
        ...
        <div class="">
          <div class="page-title">
            <div class="title_left">
              <h3>
                    Detailed Statistics - Indonesia News
			...
			<div class="col-md-6 col-sm-6 col-xs-12">
              <div class="x_panel">
				...
				<div class="x_content">
					...
					<form action="detailed_statistics.php" method="GET">
						From 
  ...
  <script src="js/datatables/tools/js/dataTables.tableTools.js"></script> -->
		...
		var raw = <?php get_analysis_data($startDate, $endDate, 1, 10000); ?>;
        var dataset = new Array();
		for(i = 0; i < raw.term.length; i++){
			dataset[i] = [
			'<a target="_blank" href="http://www.google.com/search?q=berita+tentang+'+raw.term[i]+'&tbm=nws">'+
			raw.term[i] +'</a>', 
			raw.count[i]];
		}
		var raw2 = <?php get_analysis_data($startDate, $endDate, 2, 10000); ?>;
        var dataset2 = new Array();
		for(i = 0; i < raw2.term.length; i++){
			dataset2[i] = [
			'<a target="_blank" href="http://www.google.com/search?q=berita+tentang+'+raw2.term[i]+'&tbm=nws">'+
			raw2.term[i] +'</a>', 
			raw2.count[i]];
		}
		var raw3 = <?php get_analysis_data($startDate, $endDate, 3, 10000); ?>;
        var dataset3 = new Array();
		for(i = 0; i < raw3.term.length; i++){
			dataset3[i] = [
			'<a target="_blank" href="http://www.google.com/search?q=berita+tentang+'+raw3.term[i]+'&tbm=nws">'+
			raw3.term[i] +'</a>', 
			raw3.count[i]];
		}
		var raw4 = <?php get_analysis_data($startDate, $endDate, 4, 10000); ?>;
        var dataset4 = new Array();
		for(i = 0; i < raw4.term.length; i++){
			dataset4[i] = [
			'<a target="_blank" href="http://www.google.com/search?q=berita+tentang+'+raw4.term[i]+'&tbm=nws">'+
			raw4.term[i] +'</a>', 
			raw4.count[i]];
		}
		var raw5 = <?php get_analysis_data($startDate, $endDate, 5, 10000); ?>;
        var dataset5 = new Array();
		for(i = 0; i < raw5.term.length; i++){
			dataset5[i] = [
			'<a target="_blank" href="http://www.google.com/search?q=berita+tentang+'+raw5.term[i]+'&tbm=nws">'+
			raw5.term[i] +'</a>', 
			raw5.count[i]];
		}
		var raw6 = <?php get_source_data($startDate, $endDate, 10000); ?>;
		...
		var raw7 = <?php get_headlines($startDate, $endDate); ?>;
